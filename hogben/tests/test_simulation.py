...
class TestSimulate:
    ...
    def test_data_streaming(self, refnx_model):
        ...
        assert np.all(np.zeros_like(simulated_datapoints)
    ...
    def test_incident_flux_data(self):
        """
        Tests that the `_incident_flux_data` function correctly returns count
        data when instruments are loaded, and raises errors when given an
        incorrect path
        """
        ...
        for instrument in ['OFFSPEC', 'SURF', 'INTER', 'POLREF']:
            ...
            assert (sim_no_pol._incident_flux_data(polarised=False).shape[1]
                    == 2)
            assert np.sum(
                sim_no_pol._incident_flux_data(polarised=False)[:, 1]) > 100
        ...
        # Test that a non-polarised instrument can't be used for a
        # polarised simulation
        with pytest.raises(FileNotFoundError):
    ...
    def test_reflectivity(self, refnx_model):
        ...
        ideal_reflectivity = sim.reflectivity(np.linspace(0.001,
                                                          0.3, 200))
